...
public class TERenderer {
    ...
    /** Render frame but hide the tiles far away from the given [X][Y].*/
    public void renderNearFrame(TETile[][] world, int X, int Y, int xVision, int yVision) {
        int numXTiles = world.length;
        int numYTiles = world[0].length;
        StdDraw.clear(new Color(0, 0, 0));
        for (int x = 0; x < numXTiles; x += 1) {
            for (int y = 0; y < numYTiles; y += 1) {
                ...
                if ((Math.abs(x-X) <= xVision / 2.0) && (Math.abs(y-Y) <= yVision / 2.0)) {
                    world[x][y].draw(x + xOffset, y + yOffset);
                } else if ((Math.abs(x-X) <= xVision / 1.5) && (Math.abs(y-Y) <= yVision / 1.5)) {
                    world[x][y].drawDarker(x + xOffset, y + yOffset);
                } else if ((Math.abs(x-X) <= xVision) && (Math.abs(y-Y) <= yVision)) {
    ...
    /** Render frame but hide the tiles far away from the given [X1][Y1] and [X2][Y2].*/
    public void renderNearFrameTwoPlayer(TETile[][] world, int X1, int Y1, int X2, int Y2, int xVision, int yVision) {
        int numXTiles = world.length;
        int numYTiles = world[0].length;
        StdDraw.clear(new Color(0, 0, 0));
        for (int x = 0; x < numXTiles; x += 1) {
            for (int y = 0; y < numYTiles; y += 1) {
                ...
                if (((Math.abs(x-X1) <= xVision / 2.0) && (Math.abs(y-Y1) <= yVision / 2.0))
                    ...
                    world[x][y].draw(x + xOffset, y + yOffset);
                } else if (((Math.abs(x-X1) <= xVision / 1.5) && (Math.abs(y-Y1) <= yVision / 1.5))
                        || ((Math.abs(x-X2) <= xVision / 1.5) && (Math.abs(y-Y2) <= yVision / 1.5))) {
                    world[x][y].drawDarker(x + xOffset, y + yOffset);
                } else if (((Math.abs(x-X1) <= xVision) && (Math.abs(y-Y1) <= yVision))
